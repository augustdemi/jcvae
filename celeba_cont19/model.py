...
class EncoderB(nn.Module):
    ...
    @expand_inputs
    def forward(self, attributes, index, num_samples=None, q=None):
        if q is None:
            q = probtorch.Trace()
...
class DecoderB(nn.Module):
    ...
    def forward(self, attributes, shared, index, q=None, p=None, num_samples=None, train=True, CUDA=False):
        shared_mean = torch.zeros_like(q['sharedB' + str(index)].dist.loc)
        shared_std = torch.ones_like(q['sharedB' + str(index)].dist.scale)
        pred = {}
        ...
        for shared_from in shared.keys():
            ...
            p.loss(
                ...
                pred_labels, attributes, name='attr' + str(index) + '_' + shared_from)
