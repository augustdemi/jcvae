...
if __name__ == "__main__":
    ...
    parser.add_argument('--ckpt_epochs', type=int, default=350, metavar='N',
...
# path parameters
MODEL_NAME = 'mnist_half-run_id%d-priv%02ddim-label_frac%s-sup_frac%s-lamb_text%s-beta1%s-beta2%s-seed%s-bs%s-wseed%s' % (
...
def test(data, encA, decA, encB, decB, epoch):
    ...
    if (epoch + 1) % 10 == 0 or epoch + 1 == args.epochs:
        util.evaluation.save_cross_mnist_half(epoch, test_data, encA, decA, encB, CUDA, MODEL_NAME,
                                              fixed_idxs=[3, 2, 1, 32, 4, 23, 21, 36, 61, 99],
                                              flatten_pixel=NUM_PIXELS)
...
if args.ckpt_epochs == args.epochs:
    ...
    util.evaluation.save_cross_mnist_half(args.epochs, test_data, encA, decA, encB, CUDA, MODEL_NAME,
                                          fixed_idxs=[3, 2, 1, 32, 4, 23, 21, 36, 61, 99],
                                          flatten_pixel=NUM_PIXELS)
