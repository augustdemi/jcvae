...
# path parameters
MODEL_NAME = '%s-run_id%d-priv%02ddim-label_frac%s-sup_frac%s-lamb_text%s-beta%s-seed%s' % (args.dataset, args.run_id, args.n_private, args.label_frac, args.sup_frac, args.lambda_text, args.beta, args.seed)
DATA_PATH = '../data'
...
def get_paired_data(paired_cnt, seed):
    ...
    for idx in total_random_idx:
        img, label, _, _ = data.dataset.__getitem__(idx)
        imgs.append(img)
