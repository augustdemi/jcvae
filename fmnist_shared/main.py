...
if __name__ == "__main__":
    ...
    parser.add_argument('--annealing_epochs', type=int, default=200, metavar='N',
...
# path parameters
MODEL_NAME = 'fmnist_shared-run_id%d-shared%02ddim-label_frac%s-sup_frac%s-lamb_text%s-beta1%s-beta2%s-seed%s-bs%s-wseed%s' % (
...
def elbo(q, pA, pB, lamb=1.0, annealing_factor=1.0, bias=1.0):
    # from each of modality
    beta1 = (1.0, 1.0, 1.0)
    beta2 = (1.0, 1.0, 1.0)

    reconst_loss_A, kl_A = probtorch.objectives.mws_tcvae.elbo(q, pA, pA['images_sharedA'], latents=['sharedA'],
...
def test(data, encA, decA, encB, decB, epoch):
    ...
    for b, (images, labels) in enumerate(data):
        if images.size()[0] == args.batch_size:
            ...
            batch_elbo, _, _ = elbo(q, pA, pB, lamb=args.lambda_text, bias=BIAS_TEST)
