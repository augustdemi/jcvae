...
class Solver(object):
    ...
    def __init__(self, args):
        ...
        # groundtruth factor labels
            ...
            latent_classes, latent_values = np.load( os.path.join(
                self.dset_dir, '3dfaces/gt_factor_labels.npy' ) )
        ...
        self.z_dim = args.n_private
        self.zS_dim = args.n_shared
        
        save_path = "/gpu2/yw632/research/Disentanglemen_pytorch/models/dsprites-dataset/factor_vae/dim_z_10/1_1_35vae/"
        
        # checkpoints
        self.ckpt_dir = os.path.join(save_path+"ckpts", self.name)

        # create dirs: "records", "ckpts", "outputs" (if not exist)
        mkdirs(save_path+"records");  mkdirs(save_path+"outputs")

        # records
        self.record_file = save_path+'records/%s.txt' % ("test_" + self.name)

        # outputs
        self.output_dir_recon = os.path.join( save_path+"outputs",
                                              "test_" + self.name + '_recon' )
        self.output_dir_synth = os.path.join(save_path+ "outputs",
                                              "test_" + self.name + '_synth' )
        self.output_dir_trvsl = os.path.join(save_path+ "outputs",
                                              "test_" + self.name + '_trvsl' )
        
        

        self.encoder = args.enc
        # self.decoder = args.dec
    ...
    def test(self):
        ...
        self.data_loader = CustomDataset(self.args)
        print('...done')
        ...
        for iteration in range(1, 100000000):
            ...
            with torch.no_grad():
                ...
                # kl loss
                loss_kl = -0.5*(1 + logvar - mu**2 - std**2).sum(1).mean()
                ...
                # total loss for vae
                vae_loss = loss_recon + self.lamkl*loss_kl + self.gamma*loss_tc
                ...
                # print the losses
                if iteration % 100 == 0:                  
                    prn_str = ( '[%d/%d] vae_loss: %.3f (recon: ' + \
                        '%.3f, kl: %.3f, tc: %.3f) | dis_loss: %.3f' ) % \
                        ( iteration, iter_per_epoch, 
                          vae_loss.item(), loss_recon.item(), loss_kl.item(), 
                          loss_tc.item(), dis_loss.item() )                    
                    print(prn_str)
                    self.dump_to_record(prn_str)
    ...
    def eval_disentangle_metric1(self):
        ...
        for ib in range(int(nsamps_agn_factor/bs)):
            ...
            mub = torch.cat([q[self.latents['private']].dist.loc.squeeze(0), q[self.latents['shared']].dist.loc.squeeze(0)],
                            dim=1)
        ...
        vars_per_factor = np.zeros([num_pairs, self.z_dim + self.zS_dim])
