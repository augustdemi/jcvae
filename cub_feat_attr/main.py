...
# path parameters
MODEL_NAME = 'cub_feat_attr-run_id%d-privA%02ddim-lamb%s-beta%s-lr%s-bs%s-wseed%s-seed%s-num_hidden%s' % (
...
encA = EncoderA(args.wseed, zPrivate_dim=args.n_privateA, zSharedAttr_dim=ATTR_DIM, num_hidden=args.num_hidden)
decA = DecoderA(args.wseed, zPrivate_dim=args.n_privateA, zSharedAttr_dim=ATTR_DIM, num_hidden=args.num_hidden)
...
def test(data, encA, decA, encB, decB, epoch):
    ...
    for b, (images, attributes, _) in enumerate(data):
        if images.size()[0] == args.batch_size:
            ...
            pB, acc = decB(attributes, shared_dist, q=q,
                           num_samples=NUM_SAMPLES)
