...
# path parameters
MODEL_NAME = 'cub-img-vae-run_id%d-bs%d' % (
...
def vae_loss(recon, orig, mu, logvar):
    recon_loss = -(torch.log(recon + EPS) * orig + torch.log(1 - recon + EPS) * (1 - orig)).sum(-1)
...
def train(data, encA, decA, optimizer):
    ...
    for b, (images, _, _) in enumerate(data):
        if images.size()[0] == args.batch_size:
            ...
            if CUDA:
                images = images.cuda()
            sample, mu, logvar = encA(images)
            recon_images = decA(sample)
            recon_images = recon_images.view(recon_images.size(0), -1)
            images = images.view(images.size(0), -1)
            loss = vae_loss(recon_images, images, mu, logvar).mean()
            loss.backward()
            optimizer.step()
            if CUDA:
                loss = loss.cpu()
...
for e in range(args.ckpt_epochs, args.epochs):
    ...
    if (e + 1) % 10 == 0 or e + 1 == args.epochs:
        save_ckpt(e + 1)
        util.evaluation.save_recon_cub_vae(e, test_data, encA, decA, CUDA, MODEL_NAME,
                                           fixed_idxs=[658, 1570, 2233, 2456, 2880, 1344, 2750, 1800, 1111, 300, 700,
                                                       1270, 2133, 2856, 2680, 1300])
        util.evaluation.save_recon_cub_vae(e, train_data, encA, decA, CUDA, MODEL_NAME,
