    ...
    def forward(self, images, shared, q=None, p=None, num_samples=None):
        ...
        for shared_name in shared.keys():
            ...
            if 'poe' in shared_name:
                hiddens = self.dec_hidden(
                    torch.cat([zPrivate, torch.pow(zShared + EPS, 1 / 3)], -1))  # zShared.shape = 1,100,10
            else:
...
class DecoderB(nn.Module):
    ...
    def forward(self, images, shared, q=None, p=None, num_samples=None):
        ...
        digit_log_weights = torch.zeros_like(
            q['sharedB'].dist.logits)  # prior is the concrete dist for uniform dist. with all params=1
